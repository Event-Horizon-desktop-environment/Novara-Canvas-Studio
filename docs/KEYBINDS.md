...
`INS` no longer places a clip — the key handler swallows it without doing anything, so use `F9` to insert. Insert/overwrite additionally answer to `,`/`.` from the Mark menu in every preset that leaves those menu bindings alone.
